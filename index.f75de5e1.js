...
function addRow() {
    ...
    const colAmount = table.rows[0].cells.length;
    for(let i = 0; i < colAmount; i++)newRow.insertCell();
    updateTable();
}
function removeRow() {
    if (table.rows.length > min) table.deleteRow(-1);
    updateTable();
}
function addCol() {
...
function removeCol() {
    if (table.rows[0].cells.length > min) for(let i = 0; i < table.rows.length; i++)table.rows[i].deleteCell(-1);
    updateTable();
}
